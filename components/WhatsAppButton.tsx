
import React from 'react';
import { EXPERT_DATA } from '../constants';

interface WhatsAppButtonProps {
  label?: string;
  className?: string;
  showMicroText?: boolean;
}

const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({ 
  label = "AGENDAR PRIMEIRA CONSULTA GRATUITA", 
  className = "",
  showMicroText = true
}) => {
  return (
    <div className={`flex flex-col items-center w-full max-w-sm mx-auto ${className}`}>
      <a 
        href={EXPERT_DATA.whatsappUrl}
        target="_blank"
        rel="noopener noreferrer"
        className="w-full gold-gradient text-black font-bold py-4 px-6 rounded-full shadow-lg transform active:scale-95 transition-all text-center text-sm md:text-base tracking-wide flex items-center justify-center gap-2"
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
        {label}
      </a>
      {showMicroText && (
        <p className="mt-3 text-gray-500 text-xs text-center">
          <span className="inline-block w-2 h-2 rounded-full bg-green-500 mr-1 animate-pulse"></span>
          Resposta rápida • Sem compromisso
        </p>
      )}
    </div>
  );
};

export default WhatsAppButton;
